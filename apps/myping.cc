...
void MyPingTimer::expire(Event *)
{
	t_->send_packets();
}

MyPingAgent::MyPingAgent() : Agent(PT_PING), seq(0), oneway(0), timer_(this)
{
...
void MyPingAgent::recv(Packet* pkt, Handler*)
{
  ...
  if ((u_int32_t)hdrip->daddr() == IP_BROADCAST) {
    if (hdr->ret == 0) {
      ...
      printf("Recv BRDCAST MyPing REPLY : at %d.%d from %d.%d\n", here_.addr_, here_.port_, hdrip->saddr(), hdrip->sport());
      Packet::free(pkt);
    }
    return;
  }
